...
function buttonSwitch(data, onClick, user) {
    if(data.data.data.email === user.email) {
        return <button className={style.active} >나와의 채팅</button>
    }
    switch (data.data.data['friend_status']) {
        case 'NONE' :
            return <button onClick={onClick} className={style.active}>친구 추가</button>
        case 'BLOCK' :
            return <button onClick={onClick} className={style.active}>차단해제 및 추가</button>
        case 'REMOVE' :
            return <button onClick={onClick} className={style.active}>친구 추가</button>
        case 'FRIEND' :
            return <button className={style.active}>채팅하기</button>
        default :
            return <button className={style.disable}>error</button>
    }
}

const AddFriend = ({onClose, onChange, id, onSubmit, isLoadingSearch, data, error, onClick, user}) => {
            ...
            <section>
                ...
                <form onSubmit={onSubmit}>
                    ...
                    <div className={style.aff_buttons}>
                        {data ?
                            buttonSwitch(data, onClick, user)
                            :
                            <button>친구 검색</button>
                        }
                    </div>
