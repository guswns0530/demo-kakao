...
const AddChattingSelectFriendJs = ({onChange, search, data, onClose, onChecked, list, onSubmit, trackPos, x, y, isLoading}) => {
    return (
        <Draggable onDrag={trackPos} position={{x, y}} bounds={"parent"}>
            <div id={style.add_chatting_popup} className={`${style.popup} ${style.focus}`} style={{top: 0, left: 0}}>
                ...
                <section>
                    ...
                    <div className={style.aff_buttons}>
                        <button onClick={onSubmit} className={list.length > 0 || !isLoading ? style.active : style.disable}>추가</button>
